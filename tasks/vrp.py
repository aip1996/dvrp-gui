...
import matplotlib
import tool.multivehicle as mv
from tool.baidu_sdk import load_G

matplotlib.use('Agg')
import matplotlib.pyplot as plt

G = load_G(os.path.abspath('.') + '\\tool' + '\\graph')
...
class VehicleRoutingDataset(Dataset):
    def __init__(self, num_samples, input_size, max_load=20, max_demand=9,
        ...
        # 自定义目标点格式
        start = torch.FloatTensor([0])
        arr = np.array(range(1, input_size + 1, 1))
        # np.random.shuffle(arr)
        arr = torch.FloatTensor(arr)
        arr = torch.cat([start, arr], 0)
        arr = arr.unsqueeze(0)
        locations = torch.cat([arr, arr], 0).unsqueeze(0)
        ...
        for i in range(num_samples - 1):
            ...
            arr = torch.cat([start, arr], 0)
            arr = arr.unsqueeze(0)
            location = torch.cat([arr, arr], 0).unsqueeze(0)
        ...
        demands = torch.randint(1, 5, dynamic_shape)
...
class VehicleRoutingDataset_optimize(Dataset):
    def __init__(self, num_samples, input_size, max_load=20, max_demand=9,
                 seed=None):
        super(VehicleRoutingDataset_optimize, self).__init__()
        ...
        # demands will be scaled to the range (0, 3)
        # demands = torch.randint(1, max_demand + 1, dynamic_shape)
        demands = torch.randint(1, 2, dynamic_shape)  # 忽略载重需求
        # demands = torch.randint(1, 2, dynamic_shape)
        demands = demands / float(max_load)
