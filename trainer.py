...
def validate_optimize(data_loader, actor, reward_fn, render_fn=None, save_dir='.',
    ...
    actor.train()
    mean_tour_length = np.around(np.mean(rewards), 3)
    return mean_tour_length, tour_indices[0].cpu().numpy().tolist()


def train_gui(var, actor, critic, task, num_nodes, train_data, valid_data, reward_fn,
    ...
    for epoch in range(5):
        ...
        for batch_idx, batch in enumerate(train_loader):
            ...
            if (batch_idx + 1) % 10 == 0:
                ...
                var[2].set('%.4f' % mean_loss)
                mean_reward = np.mean(rewards[-100:])
                var[3].set('%.4f' % mean_reward)
...
def train_vrp_gui(var, args):
    ...
    if args.checkpoint:
        ...
        print("load model success! path = ", path)

    if not args.test:
        train_gui(var, actor, critic, **kwargs)
    ...
    print('Average tour length: ', tour_length, tour_index)
...
def optimization(var, cfg):
    ...
    parser.add_argument('--train-size', default=10, type=int)
    ...
    test_dir = 'test'
    test_loader = DataLoader(test_data, args.batch_size, False, num_workers=0)
    tour_length, tour = validate_optimize(test_loader, actor, vrp.reward, vrp.render, test_dir, num_plot=5)
    var[0].set(tour_length)
    var[1].set(tour)

    print('Average tour length: ', tour_length)


def begin_train(var, cfg):
    parser = argparse.ArgumentParser(description='Combinatorial Optimization')
    parser.add_argument('--seed', default=12345, type=int)
    parser.add_argument('--checkpoint', default=None)
    parser.add_argument('--test', action='store_true', default=False)
    parser.add_argument('--task', default='vrp')
    parser.add_argument('--nodes', dest='num_nodes', default=cfg[0], type=int)
    parser.add_argument('--actor_lr', default=cfg[1], type=float)
    parser.add_argument('--critic_lr', default=cfg[2], type=float)
    parser.add_argument('--max_grad_norm', default=2., type=float)
    parser.add_argument('--batch_size', default=cfg[3], type=int)
    parser.add_argument('--hidden', dest='hidden_size', default=128, type=int)
    parser.add_argument('--dropout', default=0.1, type=float)
    parser.add_argument('--layers', dest='num_layers', default=1, type=int)
    parser.add_argument('--train-size', default=cfg[4], type=int)
    parser.add_argument('--valid-size', default=cfg[5], type=int)

    args = parser.parse_args()

    if args.task == 'vrp':
        train_vrp_gui(var, args)
    else:
